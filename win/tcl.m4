...
AC_DEFUN([SC_WITH_PCRE], [
    ...
    AC_CACHE_VAL(ac_cv_c_pcre,[
		...
		if test -f "${with_pcre}/pcre.h" -a \
			\( -f "${with_pcre}/.libs/libpcre.so" -o \
			  -f "${with_pcre}/.libs/libpcre.a" \); then
		    ac_cv_c_pcre=`(cd ${with_pcre}; pwd)`
		    PCRE_INCLUDE="-I${ac_cv_c_pcre}"
		    PCRE_LIBS="-L${ac_cv_c_pcre}/.libs -lpcre"
		elif test -f "${with_pcre}/include/pcre.h" -a \
...
# Results:
#	Adds the following arguments to configure:
#		--enable-pcre=yes|static|default|static,default|no|pcre
#
#------------------------------------------------------------------------
...
AC_DEFUN([SC_ENABLE_PCRE], [
	...
	if test "$pcre_ok" = "static" ; then
	    AC_MSG_RESULT([pcre enabled (static)])
	    AC_DEFINE(PCRE_STATIC, 1, [Link PCRE statically?])
	    AC_DEFINE(HAVE_PCRE, 1, [Do we enable PCRE interfaces?])
	elif test "$pcre_ok" = "static,default" ; then
	    AC_MSG_RESULT([pcre enabled (static,default)])
	    AC_DEFINE(PCRE_STATIC, 1, [Link PCRE statically?])
	    AC_DEFINE(USE_DEFAULT_PCRE, 1, [Use PCRE as default RE?])
	    AC_DEFINE(HAVE_PCRE, 1, [Do we enable PCRE interfaces?])
	elif test "$pcre_ok" = "default" ; then
	    AC_MSG_RESULT([pcre enabled (default)])
	    AC_DEFINE(USE_DEFAULT_PCRE, 1, [Use PCRE as default RE?])
	    AC_DEFINE(HAVE_PCRE, 1, [Do we enable PCRE interfaces?])
